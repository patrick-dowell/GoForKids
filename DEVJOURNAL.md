...
- [ ] ⚠️ **Bot passes during ko fights — can throw a won game (9×9 + 19×19).** GAME-BREAKING. **Patrick saw it on iPhone (direct Xcode build) = the on-device bridge path** (2026-06-26). Traced the whole on-device chain and the obvious cause is NOT it — the move-history plumbing is correct end-to-end: `requestAIMove` builds the real sequence and passes `movesForBridge` (gameStore.ts:904 + `buildBridgeMovesFromGame`); the Swift bridge **replays it via legal `play` commands** (`KataGoBridge.swift:104-112` — `clear_board`/`boardsize`/`komi`/`kata-set-rules japanese`, then `play <color> <pt>` per move), so KataGo's own board *is* ko-aware; and `Board.clone()` preserves `koPoint` + `positionHistory` (Board.ts:43-44), so the selector's `isLegal` filter correctly rejects the recapture. So the device cause is subtler — candidates: (a) **stale JS bundle** (the Xcode Run Script's `npm run build` didn't pick up the history fix — check the build date); (b) a **`_game` ↔ localGameRouter board desync** so `movesForBridge` (from `_game`) doesn't match the analyzed `state.board` (from the router) → KataGo's replayed ko state is wrong; (c) a **`play` rejected mid-replay** (a move legal under the app's positional-superko but not KataGo's japanese ko) silently desyncing KataGo's board (the Swift loop ignores the GTP response); (d) at low profile visits (6–10) KataGo genuinely finds **no good ko threat** and pass looks comparable. **Diagnostic = device repro with the Xcode console open:** during a ko, look for `[Bridge] analyze: … moves=N …` (is N the full history?), `[getAIMoveViaBridge] engine rejected KataGo's pick … passing instead` (→ filter/replay mismatch), and the selector's `dropped N illegal candidate(s) (likely ko)`. **Separately, the WEB/Render path has its own ko hole:** `backend/app/ai/move_selector.py` sends KataGo an **empty `moves` list** (move_selector.py:338-363 — `PASS: all candidates illegal`), so web users are ko-blind regardless; fix = send the server-side move history to the backend analysis too.
