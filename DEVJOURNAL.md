...
## Known Bugs (observed in play, 2026-05-05 / iPad pass 2026-05-07)
...
- [ ] **Sound stops working after several games (restart fixes)** — observed 2026-05-07, iPad-only so far. [SoundManager.ts](frontend/src/audio/SoundManager.ts) caches a single `AudioContext` and `masterGainNode` for the app's lifetime; cosmic pack creates 4–5 OscillatorNodes per capture (and 2 per placement). Two viable hypotheses: (a) iOS WKWebView AudioContext enters the `'interrupted'` state (e.g. screen-lock, Siri, ringer toggle) and `resumeAudio()` ([:104](frontend/src/audio/SoundManager.ts:104)) only handles `'suspended'`; (b) accumulated short-lived AudioNodes hit an internal iOS WebKit ceiling — fewer-and-pooled sources or switching cosmic placement to the sample-buffer path (like classic) would reduce pressure. Diagnostic: log `audioCtx.state` on each `resumeAudio()` call and after N games to see which it is

> Three of the four iPad-specific bugs from the 2026-05-07 playtest pass are now closed in code. The audio-death bug (#1) is still open, awaiting a repro that captures `audioCtx.state` at the moment sound dies. iPad must be rebuilt from Xcode to pick up the bundled frontend changes (Finish Game, Resign disable, rapid-click gate).
