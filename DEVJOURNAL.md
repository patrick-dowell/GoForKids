...
**Addendum — 888P9NXK verified same session: undo bug RULED OUT as its
cause; the §5 selector-log capture named the real path.** Pulled the
payload from `GET /api/uploads/888P9NXK`. It IS a handicap game (19×19
HA[2], komi 0.5, casual vs 18k, B+100.5, 290 moves) — but the selector
log shows `capture=v2 … bridge=yes`: the whole game ran on-device, where
the backend board never feeds analysis, so today's backend fix is
unrelated to this incident (it stands on its own merits). The log names
the pass mechanism outright: move 235 =
`[getAIMoveViaBridge] engine rejected KataGo's pick (5,17): Ko violation
— passing instead` — and (5,17) is exactly `rf`, the ko recapture in the
SGF's corner fight (`W[rf] B[rg] W[] B[sf] W[]`). So: the known
koSIMPLE-vs-superko mismatch, surfacing through the **commit-time catch
in client.ts (getAIMoveViaBridge:404, finishMoveViaBridge has a twin)**
— a pass path Option B never covered (B patched the *selector's*
filtered-empty fallback; this pick sailed through selection and died at
`api.playMove`). Fix shape: extend B's legal-fallback to the catch —
on rejection, re-select excluding the rejected point instead of passing
(root fix A still the real answer). One loose end: the pass at move 233
committed with NO reason line in the log — either a third unlogged pass
path or a capture gap; check while in there. Capture-first vindicated:
the first instrumented field repro classified itself, exactly as §2
designed. Details + fix plan logged in MILESTONE §2.
...
- [x] **Download SGF doesn't work on iOS** — FIX SHIPPED 2026-07-02 (pending device validation). Exactly the fix shape predicted: a `shareSGF` bridge command (KataGoBridge.swift — writes a temp .sgf, presents `UIActivityViewController` with an iPad popover anchor; JS shim + `window.kataGo.shareSGF`) and `replayStore.downloadSGF` goes bridge-first with the web Blob flow as fallback (also the fallback when an older native build lacks the handler — the unknown-cmd rejection triggers it). Button reads "Share SGF" in the app, "Download SGF" on web. Both Swift copies updated (`ios/swift/` + the vendored project). Validate on device: share sheet opens, AirDrop/Files/other-app export works. TestFlight beta 2026-05-14 original: WKWebView doesn't handle the `Blob` URL + `<a download>` flow.
