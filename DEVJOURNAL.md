...
## Known Bugs (observed in play, 2026-05-05 / iPad pass 2026-05-07 / TestFlight beta 2026-05-14 → 2026-05-15)
...
- [ ] **"Play" mode game-end modal: not dismissible + no score breakdown** — TestFlight beta 2026-05-14. The new ranked-Play-mode "You Win" modal lacks the standard game-end modal's score breakdown rows AND has no Close control, so the player can't dismiss it to inspect the final board / count. Likely fix: either add a Close button + the score-breakdown rows, or reuse the standard game-end modal with a Play-mode variant flag (promotion banner stacked on top of the normal score lines). Lives in the auto-play / ranked-Play game-end path added in Session 18.
- [ ] **Library replay: playback controls misaligned on iPhone portrait** — TestFlight beta 2026-05-14. Bottom playback-speed row is slightly clipped, and the settings (gear) button hovers above the move slider instead of sitting in-line with the playback controls. Portrait-narrow CSS issue in the replay viewer; gear should drop down into the playback control row, and the bottom safe-area / padding needs another pass.
- [ ] **Close button in replay routes to a new game instead of home** — TestFlight beta 2026-05-14. Repro: start a game → open Library → tap a replay → tap Close. Lands on a fresh game screen rather than the home menu. Likely a navigation back-stack issue — replay's close handler pops to the prior view (the in-progress game) rather than routing explicitly to home.
...
- [ ] **Profile page not scrollable in iPhone portrait** — TestFlight beta 2026-05-14. Profile view scrolls fine in landscape but is locked in portrait on iPhone. Likely a CSS issue where a portrait-only parent has `overflow: hidden` or a height constraint (e.g. `100vh` minus a too-large header) that prevents the inner content from overflowing into a scroll region. Landscape's taller working height masks the issue.
- [ ] **5×5 tutorial: White bot freezes when every move is suicide** — TestFlight beta 2026-05-15. Confirmed scenario: W reached a position where every remaining empty intersection was a suicide play — i.e. the legal-move set is truly empty, not just all-filtered-out. The bot stops playing and the game hangs; Pass / Resign / Finish Game don't recover. Expected behavior per design intent: W resigns and the lesson ends cleanly with a win for the student. (Strict Go rules would say pass, not resign — both bots passing ends the game by area scoring. But for tutorial UX, an explicit resign reads cleaner to the student than a silent pass-pass-end. Worth confirming the intended teaching beat before implementing.) Likely root cause: bot move-selection returns null on empty legal-move set and the calling loop awaits a move forever. Fix shape: in the bot move pipeline, when legal moves are empty, trigger either an explicit resign or a pass (per design choice) rather than returning null. Audit whether this can also hang on larger boards in late-game positions.
