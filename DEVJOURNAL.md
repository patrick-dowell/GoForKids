...
- [ ] ⚠️ **Bot passes during ko fights — can throw a won game (9×9 + 19×19).** GAME-BREAKING. **Root cause (web/backend path):** `backend/app/ai/move_selector.py` sends KataGo an **empty `moves` list** (board only, no history), so KataGo is **ko-blind**, recommends the **illegal ko recapture**, the `_is_legal` filter drops it (logs `dropped N illegal candidate(s) (likely ko)`), and when no legal non-pass candidate survives the bot **passes** (`PASS: all candidates illegal`, move_selector.py:359-363). KataGo also "wastes" its search budget on the recapture it thinks is best, so any surviving ko-threat candidates are too low-visit to trust → pass-detection bails too. The **on-device bridge** path was already fixed — `requestAIMove` passes real `movesForBridge` (gameStore.ts:904) so KataGo tracks ko — so this primarily bites the **web/Render** path. **Fix shape:** build the GTP move sequence server-side and pass it to the backend analysis (same fix as the bridge; the backend has the full game state). Secondary on-device suspect if it recurs there: positional-superko (app engine) vs KataGo's ko rule disagreeing on a multi-stage/superko, filtering a move KataGo thinks legal → fall-through to pass. **Needs a repro to confirm which path Patrick hit (web vs device).**
- [ ] **Bot fills neutral dame instead of passing after the player passes (esp. 19×19).** Wastes the player's time; not game-breaking. Analysis runs under **japanese rules** (dame = 0 pts) and the post-opponent-pass **settle path** (moveSelector.ts `opponentPassed` + `SETTLE_VISITS=100`; move_selector.py mirror) is supposed to surface pass. But the selector only passes if the pass candidate got **≥ max(4, best.visits/10) visits** AND the best move beats pass by < `pass_threshold`. On **19×19**, `SETTLE_VISITS=100` is likely too shallow for pass to accumulate enough visits among the many dame candidates, so the guard is skipped and the bot plays a dame fill (scoreLead ≈ pass under japanese, a hair higher from search noise) — then passes once the dame run out: exactly "fills neutrals, gains nothing, then passes." **Fix directions:** scale `SETTLE_VISITS` with board area, and/or a japanese-rules "score-neutral → just pass" heuristic in the settle path (if the best move's scoreLead gain over the current position is ~0, pass regardless of the pass candidate's visit count). Overlaps the milestone §7 settle-fill item (validated on 9×9, doesn't scale to 19×19).
