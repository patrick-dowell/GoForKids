...
- **9k Boulder** — shipped v5 (2026-04-24): v1 parameters (visits=80) + universal pass/clarity fixes. Match rate 20.4% exact / 49% same-area vs real 9k Fox games. Loses 88% vs 6k v4 at even — H3 overcompensates (9k+3 wins 88%). Both bots are calibrated by match-rate vs real Fox data; bot-vs-bot handicap theory is broken because the universal safety fixes make both bots more robust than real humans at their nominal ranks.
- **6k Ember** — shipped v5 (2026-04-24): visits=120, mistake_freq=0.22, max_point_loss=9. v4 (visits=150, mistake=0.32) matched 15k baseline on match-rate (25% exact) but playtest said "a bit stronger than 6k" with "too drastic" mistakes. v5 relies on natural tactical shallowness for imperfection. **Known limitation** (see AI_CALIBRATION.md): Phase 1 can't simulate *type* of human mistakes (wrong direction, missed big point), only magnitude. Phase 2 (rank-conditioned NN) is the path to natural-feeling errors.
