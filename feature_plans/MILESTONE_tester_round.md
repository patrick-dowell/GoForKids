...
**§3 addendum 2 (S50, 2026-07-06 night): the saga resolved — the problem was never the 9k.** Patrick's playtest loop + his gap probes showed the whole old 9×9 midsection (old-9k/12k/6k) pressed against the **b28 policy ceiling** (12k beat 6k 3/5 EVEN at komi 0.5; +H3 swept 5/5) — rungs differed mainly in dice, and per-game strength swung ~3 ranks on identical knobs. Sampler v3 shipped (min_loss floor + read cooldown, Patrick's streak insight; dormant for now) and produced the week's key finding: **identical per-move histograms, 45-pt game gap — mistake TIMING/correlation decides games, and only game results measure it.** Resolution = Patrick's re-seat (S50c): 9k := old-12k verbatim (the felt-validated rung → new Boulder), 12k := midpoint(15k, 9k), 6k unchanged. Untested at birth by design — his device pass gates everything; then the automated sweep. Detail: DEVJOURNAL S50.

**§3 addendum (S49+S49b, 2026-07-06): the 9k rung reopened, fixed twice.** Patrick (2d) went 2-4 vs 9k EVEN; local baseline showed the pair INVERTED (9k beat the 6k anchor 5/8). Round 1 (rr .12→.10, temp 2.1→2.2, λ .25→.30) restored local ordering but **still slaughtered Patrick B+75 (upload 5QDYHMFG)** — measured 61% near-opt / median 0.21 = "the 6k bot with random mistakes thrown in" (his words, exactly the distribution). Round 2 findings: b28's 9×9 priors (~0.95-0.98) make policy_temp a dead dial; λ sets mistake frequency but not the baseline; **visits is the baseline dial** → 9k = visits 16→12 + λ .45. Distribution now ON the human 9k column (48%/0.59 vs 49%/0.51). 12k untouched (Patrick beats it 3/4). Detail: DEVJOURNAL S49/S49b. **Process shift: Patrick's play is the acceptance instrument** — both local instruments (bot-vs-bot cells, histograms-in-tournament-context) passed profiles his games refuted; the pre-handoff gate is now the ~2-min distribution check + the new per-game knob stamp (`rr= temp= lapse= mf= v=` in every game log). Closes on Patrick's local-build feel; watch 9k-vs-12k ordering.
